  ...
  async function fetchUsersList(walletAddress: string, filterRange: string, page: string, reset: boolean) {
    ...
    try {
      ...
      setUsersData(prevUsersData => ({
        ...res,
        leaderboard: reset ? res.leaderboard : [...prevUsersData.leaderboard, ...res.leaderboard],
    ...
    <div className="max-w-full h-[100vh] mx-auto flex flex-col">
      ...
      <div className='flex flex-col px-[10px] py-3'>
        ...
        <div className='flex flex-col items-center h-[10vh]'>
          ...
          <div className="flex bg-[#020304] w-full max-w-md mx-auto rounded-full">
            {['Today', 'This Week', 'This Month'].map((tab, index) => (
              ...
              ${activeTab === tab ? 'bg-[#0B688A] text-[#020304]' : 'text-[#0B688A] bg-[#020304]'
                  }`}
        ...
        <div className='flex flex-col justify-center items-center px-3 mt-1'>
          <div className="overflow-y-auto h-[70vh] max-h-screen hide-scrollbar" onScroll={listenScrollEvent} ref={userlist}>
            {
                ...
                <span key={index}>
                  <div className={`${userInfos.userId === address
                    ...
                    <div className="flex flex-col items-start ml-3 col-span-7 justify-start">
                      <div className="text-white text-xl font-bold">{userInfos.userId.slice(0, 5)}...{userInfos.userId.slice(-4)}</div>
                      <div className='flex mt-2'>
                        <img src='/image/icon/coin_small_leaderboard.svg' alt='coin_small' />
                        <div className="text-white font-bold ml-2">{userInfos.totalScore}</div>
                      </div>
                    </div>
                    {
